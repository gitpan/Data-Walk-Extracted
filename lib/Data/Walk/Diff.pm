...
BEGIN{
	if( $ENV{ Smart_Comments } ){
		...
		### Smart-Comments turned on for Data-Walk-Diff ...
	}
}
...
$| = 1;
my $graft_keys = {
    primary_ref => 'compare_ref',
    secondary_ref => 'base_ref',
};
my 	$wait;
...
has 'diff_both_ways' =>(
    is			=> 'ro',
    isa			=> Bool,
    writer		=> 'set_diff_both_ways',
	reader		=> 'get_diff_both_ways',
	predicate	=> 'has_diff_both_ways',
	clearer		=> 'clear_diff_both_ways',
);

###############  Public Methods  ########################################################

sub diff_data{#Used to convert names
